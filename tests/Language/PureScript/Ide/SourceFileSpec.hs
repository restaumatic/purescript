...
valueFixity =
  P.ValueFixityDeclaration
    ann1
    (P.Fixity P.Infix 0)
    (P.mkQualified_ P.ByNullSourcePos (Left (P.Ident "")))
    (P.OpName "<$>")
typeFixity =
  P.TypeFixityDeclaration
    ann1
    (P.Fixity P.Infix 0)
    (P.mkQualified_ P.ByNullSourcePos (P.ProperName ""))
    (P.OpName "~>")
  ...
  where
    ideState = emptyIdeState `volatileState`
      [ ("Test",
         ...
         , ideValueOp "<$>" (P.mkQualified_ P.ByNullSourcePos (Left "")) 0 Nothing Nothing
           `annLoc` valueOpSS
         , ideTypeOp "~>" (P.mkQualified_ P.ByNullSourcePos "") 0 Nothing Nothing
           `annLoc` typeOpSS
         ])
      ]
