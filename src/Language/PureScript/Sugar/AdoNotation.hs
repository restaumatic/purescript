...
desugarAdo d =
  ...
  pure' ss m = Var ss (mkQualified_ (byMaybeModuleName m) (Ident C.S_pure))

  map' :: SourceSpan -> Maybe ModuleName -> Expr
  map' ss m = Var ss (mkQualified_ (byMaybeModuleName m) (Ident C.S_map))

  apply :: SourceSpan -> Maybe ModuleName -> Expr
  apply ss m = Var ss (mkQualified_ (byMaybeModuleName m) (Ident C.S_apply))
  ...
  go ss (yield, args) (DoNotationBind binder val) = do
    ...
    let abs = Abs (VarBinder ss ident)
                  (Case [Var ss (mkQualified_ ByNullSourcePos ident)]
