...
desugarGuardedExprs ss (Case scrut alternatives)
  | not $ all isTrivialExpr scrut = do
    ...
    (scrut', scrut_decls) <- unzip <$> forM scrut (\e -> do
      scrut_id <- freshIdent'
      pure ( Var ss (mkQualified_ ByNullSourcePos scrut_id)
  ...
  let
    ...
    desugarAltOutOfLine alt_binder rem_guarded rem_alts mk_body
      | Just rem_case <- mkCaseOfRemainingGuardsAndAlts = do
        ...
        let
          goto_rem_case :: Expr
          goto_rem_case = Var ss (mkQualified_ ByNullSourcePos rem_case_id)
...
desugarAbs = flip parU f
  ...
  replace (Abs binder val) = do
    ident <- freshIdent'
    return $ Abs (VarBinder nullSourceSpan ident) $ Case [Var nullSourceSpan (mkQualified_ ByNullSourcePos ident)] [CaseAlternative [binder] [MkUnguarded val]]
...
makeCaseDeclaration ss ident alternatives = do
  ...
  let vars = map (Var ss . mkQualified_ ByNullSourcePos . snd) args
